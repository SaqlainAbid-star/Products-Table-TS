...
fetch(apiUrl)
  ...
  .then(data => {
    ...
    const dataArray = data.products.map((products: Product) => [products.id, products.title,products.price,products.brand,products.category]);
    ...
    const headerLabels = ['ID','Title', 'Price', 'Brand','Category'];
    ...
    const tableContainer = document.getElementById('table-container')! as HTMLInputElement;
